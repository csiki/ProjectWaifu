...
public class WaifuMenuBehavior extends Behavior {
	...
	StringTyped nameTypedWindu = null;
	StringTyped nameTypedSamuel = null;
	StringTyped nameTypedBmf = null;
	...
	public void condition(UserActionFactory UAF) {
		...
		// windu
		nameTypedWindu = UAF.createStringTyped("windu");
		nameTypedWindu.activate(this);
		
		// samuel
		nameTypedSamuel = UAF.createStringTyped("samuel");
		nameTypedSamuel.activate(this);
		
		// bad mother fucker
		nameTypedBmf = UAF.createStringTyped("bmf");
		nameTypedBmf.activate(this);
	}
	...
	public void consequent(CounterActionFactory CAF) {
		...
		String question = "Don't give me that grin, say what yo want me to do!";
		if (mName != null) {
			question = "What yo up to, " + mName + "?";
		}
		...
		menu.addOption("Read my comment nigga!");
		menu.addOption("Samuel, quote some wise shit!");
		menu.addOption("Just talkin man, just talkin.");
		menu.addOption("Future events?");
	...
	private void readComment(CounterActionFactory CAF) {
		CloudComment cc = CAF.createCloudComment("Highlight that shit!");
		SkinSwitch ss = CAF.createSkinSwitch(Emotion.happy.code);
		...
		if (!commentHed.isTextHighlighted()) {
			...
			if (wName != null) {
				if (text.contains(wName.toLowerCase())) {
					cc.setComment("Whatya talking about me behind my back nigga?");
			...
			if (favMov != null) {
				if (text.contains(favMov.toLowerCase())) {
					cc.setComment("That's your favourite movie, " + favMov + ", you mentioned! That movie sucks man, get a life.");
			...
			if (favSpot != null) {
				if (text.contains(favSpot.toLowerCase())) {
					cc.setComment("That's your favourite spot on earth you mentioned, " + favSpot + "!");
					ss.setSkin(Emotion.happy.code);
			...
			if (musicRecently != null) {
				if (text.contains(musicRecently.toLowerCase())) {
					cc.setComment("The music you listening to nowadays, " + musicRecently + ", is mentioned! That's ok I guess.");
					ss.setSkin(Emotion.happy.code);
			...
			if (crushName != null) {
				if (text.contains(crushName.toLowerCase())) {
					cc.setComment("Yo babe's name, hah!");
					ss.setSkin(Emotion.happy.code);
			...
			String[] reactions = {
					"Good shit " + mName + "!",
					"Shit's got real, " + mName + "!",
					mName + ", niggas gonna be like whaaaa?",
					"Aha, it's ok.",
					"Bring it on " + mName + ", bring it on!"};
	...
	private void somethingPositive(CounterActionFactory CAF) {
		...
		SkinSwitch ss = CAF.createSkinSwitch(Emotion.extra1.code);
	...
	private void someTalking(CounterActionFactory CAF) {
		...
		String[] questions = {
				"What ya watching lately? Pulp fiction is a must!",
				"Favourite place?",
				"What ya bangin your ass to, what ya listening?",
				"Do you know anyone who turns you on? I need a name!"};
		String[] questionsNTime = {
				"What ya watching lately? I know it was " + favMovie + " last time I asked you! Something new?",
				"Favourite place? I know " + favSpot + ", but that's all?",
				"I know you listening to " + musicRecently + " recently, got anything new?",
				"Last time we talked you was in that " + crushName + " thing. Any updates? Maybe a new one?"};
		...
		if (answer == null) {
			...
			cc.setComment("Press cancel again.. Press cancel again, I dare you, I double dare you motherfuker!");
			...
			ss.setSkin(Emotion.happy.code);
			cc.setComment("Ok, seems interesting.");
	...
	private void eventsInMind(CounterActionFactory CAF) {
		...
		if (events == null) {
			ss.setSkin(Emotion.sleepy.code);
			cc.setComment("Nothing, I'd know about!");
			...
			if (events.size() == 0) {
				...
				ss.setSkin(Emotion.facingaway.code);
				cc.setComment("Yeah there's some actually..");
