...
public class EventStore extends Behavior {
	...
	public void consequent(CounterActionFactory CAF) {
		...
		CloudComment cloudComment = CAF.createCloudComment("Get yo'r ass together! Whatya doing today? Is there anything?");
		...
		yesnoRadioBtn.addOption("Yeah");
		yesnoRadioBtn.addOption("Naaah");
		...
		if (selectedIndex == -1) {
			...
			cloudComment.setComment("Press cancel again.. Press cancel again, I dare you, I double dare you motherfuker!");
			...
			while (askFurther) {
				...
				cloudComment.setComment("What ya calling that event?");
				...
				cloudComment.setComment("Is it for today?");
				...
				if (selectedIndex == -1) {
					...
					// it's for today
					// ask for hour
					cloudComment.setComment("What hour notify yo'r ass (0-23)?");
					...
					if (eventHour.getInput() == null) {
						...
						try {
							...
							cloudComment.setComment("That's not a number, yo mothafucker!");
							skinSwitch.setSkin(Emotion.mad.code);
						...
						cloudComment.setComment("When exactly (minute 0-59)?");
						...
						if (eventMinute.getInput() == null) {
							...
							// say ok
							cloudComment.setComment("Got that " + eventName + " thing!");
					...
					cloudComment.setComment("What month (1-12)?");
					...
					if (eventMonth.getSelectedIndex() == -1) {
						...
						// day
						cloudComment.setComment("What day (1-31)?");
						...
						if (eventDay.getInput() == null) {
							...
							// hour
							cloudComment.setComment("What hour (0-23)?");
							InputBox eventHour = CAF.createInputBox();
				...
				skinSwitch.setSkin(Emotion.talking.code);
				cloudComment.setComment("Any more of this shit?");
			...
			if (rejected) {
				skinSwitch.setSkin(Emotion.mad.code);
				cloudComment.setComment("Get the fuck out of my face, nigga!");
			...
			cloudComment.setComment("Shit's just got real.");
